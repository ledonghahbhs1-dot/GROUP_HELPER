...
async def cmd_settings(message: Message, bot: Bot):
    ...
    if message.chat.type in ["private"]:
        if not is_user_allowed_private(message.from_user):
            return
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Vui lòng sử dụng lệnh này bên trong nhóm mà bạn muốn cấu hình!"))
        return

    if not await is_user_admin(chat_id, user_id, bot):
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Bạn phải là Quản trị viên của nhóm để sử dụng lệnh này!"))
        return
    ...
    await message.answer(emoji_mgr.format_msg(text), reply_markup=keyboard, parse_mode="HTML")
...
async def cmd_warn(message: Message, command: CommandObject, bot: Bot):
    ...
    if not message.reply_to_message or not message.reply_to_message.from_user:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Vui lòng reply tin nhắn của người cần cảnh cáo!"))
        return

    target = message.reply_to_message.from_user
    if target.id == (await bot.get_me()).id:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Không thể cảnh cáo bot!"))
        return

    if await is_user_admin(chat_id, target.id, bot):
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Không thể cảnh cáo Quản trị viên khác!"))
        return
...
async def cmd_unwarn(message: Message, bot: Bot):
    ...
    if not message.reply_to_message or not message.reply_to_message.from_user:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Vui lòng reply tin nhắn của người cần xoá cảnh cáo!"))
        return
...
async def cmd_mute(message: Message, command: CommandObject, bot: Bot):
    ...
    if not message.reply_to_message or not message.reply_to_message.from_user:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Vui lòng reply tin nhắn của người cần cấm chat kèm thời gian (vd: <code>/mute 30m Spam</code>)!"))
        return

    target = message.reply_to_message.from_user
    if await is_user_admin(chat_id, target.id, bot):
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Không thể cấm chat Quản trị viên!"))
        return
    ...
    try:
        ...
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Lỗi khi cấm chat: {e}"))
...
async def cmd_unmute(message: Message, bot: Bot):
    ...
    if not message.reply_to_message or not message.reply_to_message.from_user:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Vui lòng reply tin nhắn của người cần mở cấm chat!"))
        return
    ...
    try:
        ...
        target_mention = f"<a href='tg://user?id={target.id}'>{html.escape(target.full_name)}</a>"
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.success} Đã gỡ cấm chat cho {target_mention}!"), parse_mode="HTML")
    except Exception as e:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Lỗi khi gỡ cấm chat: {e}"))
...
async def cmd_kick(message: Message, command: CommandObject, bot: Bot):
    ...
    if not message.reply_to_message or not message.reply_to_message.from_user:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Vui lòng reply tin nhắn của người cần kick!"))
        return

    target = message.reply_to_message.from_user
    if await is_user_admin(chat_id, target.id, bot):
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Không thể kick Quản trị viên!"))
        return
    ...
    try:
        ...
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Lỗi khi kick: {e}"))
...
async def cmd_ban(message: Message, command: CommandObject, bot: Bot):
    ...
    if not message.reply_to_message or not message.reply_to_message.from_user:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Vui lòng reply tin nhắn của người cần cấm vĩnh viễn!"))
        return

    target = message.reply_to_message.from_user
    if await is_user_admin(chat_id, target.id, bot):
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Không thể ban Quản trị viên!"))
        return
    ...
    try:
        ...
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Lỗi khi cấm thành viên: {e}"))
...
async def cmd_unban(message: Message, command: CommandObject, bot: Bot):
    ...
    if not command.args or not command.args.strip().isdigit():
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Vui lòng nhập User ID cần gỡ cấm (vd: <code>/unban 123456789</code>)!"))
        return

    target_id = int(command.args.strip())
    try:
        await bot.unban_chat_member(chat_id, target_id)
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.success} Đã gỡ cấm thành công cho ID <code>{target_id}</code>!"), parse_mode="HTML")
    except Exception as e:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Lỗi khi gỡ cấm: {e}"))
...
async def cmd_addword(message: Message, command: CommandObject, bot: Bot):
    ...
    if not await is_user_admin(chat_id, user_id, bot):
        return
    if not command.args:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Cú pháp: <code>/addword [từ cấm]</code>"))
        return
    word = command.args.strip().lower()
    success = await db.add_banned_word(chat_id, word, user_id)
    if success:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.success} Đã thêm từ <b>{html.escape(word)}</b> vào danh sách cấm của nhóm!"), parse_mode="HTML")
    else:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Từ này đã có trong danh sách cấm!"))
...
async def cmd_delword(message: Message, command: CommandObject, bot: Bot):
    ...
    if not await is_user_admin(chat_id, user_id, bot):
        return
    if not command.args:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Cú pháp: <code>/delword [từ cần xoá]</code>"))
        return
    word = command.args.strip().lower()
    success = await db.remove_banned_word(chat_id, word)
    if success:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.success} Đã xoá từ <b>{html.escape(word)}</b> khỏi danh sách cấm!"), parse_mode="HTML")
    else:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Không tìm thấy từ này trong danh sách cấm!"))
...
async def cmd_listwords(message: Message, bot: Bot):
    ...
    if not words:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.shield} Nhóm đang dùng danh sách từ cấm mặc định."))
        return
...
async def cmd_addlink(message: Message, command: CommandObject, bot: Bot):
    ...
    if not await is_user_admin(chat_id, user_id, bot):
        return
    if not command.args:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Cú pháp: <code>/addlink [tên miền]</code> (vd: <code>/addlink youtube.com</code>)"))
        return
    domain = command.args.strip().lower()
    success = await db.add_whitelist_link(chat_id, domain)
    if success:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.success} Đã thêm <b>{html.escape(domain)}</b> vào whitelist!"), parse_mode="HTML")
    else:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Tên miền này đã có trong whitelist!"))
...
async def cmd_dellink(message: Message, command: CommandObject, bot: Bot):
    ...
    if not await is_user_admin(chat_id, user_id, bot):
        return
    if not command.args:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.warn} Cú pháp: <code>/dellink [tên miền]</code>"))
        return
    domain = command.args.strip().lower()
    success = await db.remove_whitelist_link(chat_id, domain)
    if success:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.success} Đã xoá <b>{html.escape(domain)}</b> khỏi whitelist!"), parse_mode="HTML")
    else:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Không tìm thấy tên miền trong whitelist!"))
...
async def cmd_listlinks(message: Message, bot: Bot):
    ...
    if not links:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.link} Whitelist liên kết đang trống (tất cả link bên ngoài đều bị chặn)."))
        return
...
async def cmd_get_emoji(message: Message):
    ...
    await message.answer(emoji_mgr.format_msg("\n".join(output_lines)), parse_mode="HTML")
...
async def cmd_set_emoji(message: Message, command: CommandObject, bot: Bot):
    ...
    if user_id not in config.OWNER_IDS and not await is_user_admin(message.chat.id, user_id, bot):
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Bạn không có quyền thực hiện lệnh này!"))
        return
    ...
    if len(parts) < 2:
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Vui lòng cung cấp cả [key] và [emoji_id]!"))
        return
    ...
    if not emoji_id.isdigit():
        await message.answer(emoji_mgr.format_msg(f"{emoji_mgr.error} Emoji ID phải là chuỗi số nguyên hợp lệ!"))
        return
